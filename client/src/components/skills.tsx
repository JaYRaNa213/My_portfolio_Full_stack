          ...
          {resumeData.skills.map((skillCategory, index) => {
            ...
            return (
              <motion.div
                ...
                viewport={{ once: true }}
                className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-6 hover:transform hover:-translate-y-1 transition-all duration-300"
              >
                <div className="flex items-center mb-4">
                  <div className={`p-3 rounded-lg mr-4 ${skillCategory.bgColor}`}>
                    <IconComponent className={`w-5 h-5 ${skillCategory.textColor}`} />
                  </div>
                  <h3 className="text-xl font-semibold text-black dark:text-white">{skillCategory.category}</h3>
                </div>
